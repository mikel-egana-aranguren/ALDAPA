(ns pipeline.EstacionesMetereologicas
  ...
  (:require [grafter.tabular :refer [_ add-column add-columns apply-columns
            ...
            [pipeline.Predicados :refer :all]
            [pipeline.TransformacionGeneral :refer :all]
            [pipeline.TransformacionesEstacionesMetereologicas :refer :all]
...
(def make-graph
 (graph-fn [{:keys [
  ...
  SigDir SigVel TemAire VelMax VelMedia Date
...
;                 [date-predicate (languageSpanish (str "Date"))]
...
(make-dataset [
 ...
 (derive-column :Date [:Fecha] organizeDate)
