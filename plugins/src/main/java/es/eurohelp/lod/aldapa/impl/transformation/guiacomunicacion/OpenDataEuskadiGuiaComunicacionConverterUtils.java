...
public class OpenDataEuskadiGuiaComunicacionConverterUtils {
    ...
    public static TripleAdder addaddress(long recordNumber, TripleAdder adder, String calle, String codigopostal,
	    String poblacion, String addressName, String cargoUri) {
	// Nada de nodos anónimos en Linked Data para direccion: no tienen por
	// que seguir la NTI ya
	// que representan una "reificacion" de una relación n-aria, no un
	// recurso

	// Hay direcciones vacias!
	if (!addressName.isEmpty()) {
	    String addressURI = EUSURI.BASEIDES.getValue() + NTITOKEN.PUBLICSECTOR.getValue() + "/"
		    + DOMAINTOKEN.PLACE.getValue() + "/" + CLASSTOKEN.ADDRESS.getValue() + "/" + addressName;
	    ...
	    // En la ontologia VCARD la poblacion es un String, pero igual se
	    // puede usar una URI de
	...
	if (apellidos.isEmpty()) {
	    // Hay lineas que no tienen nombre, y entonces cuyando accedemos a
	    // nombre aparece el cargo
	    if (!forbiddenStrings.contains(nombre)) {
		cargoUri = EUSURI.BASEIDES.getValue() + NTITOKEN.PUBLICSECTOR.getValue() + "/"
			+ DOMAINTOKEN.STAFF.getValue() + "/" + CLASSTOKEN.PERSON.getValue() + "/"
			+ URIUtils.urify(null, null, nombre);
	    } else {
		...
		cargoUri = EUSURI.BASEIDES.getValue() + NTITOKEN.PUBLICSECTOR.getValue() + "/"
			+ DOMAINTOKEN.ENTITY.getValue() + "/" + CLASSTOKEN.PERSON.getValue() + "/"
			+ URIUtils.urify(null, null, nombre + "-" + apellidos);
	    } else {
